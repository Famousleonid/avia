...
class MarketingTest extends TestCase
{
    ...
    public function test_marketing_customer_profile_can_be_updated_and_listed(): void
    {
        ...
        $this->actingAs($admin)
            ...
            ->assertSee('top: 27px !important;', false)
            ->assertDontSee('top: 31px !important;', false)
            ...
            ->assertSee('data-marketing-media-drag-handle', false)
            ->assertSee('is-window-open', false)
            ->assertSee('syncMarketingFilesWindowButton', false)
            ->assertSee('beginMarketingMediaDrag', false)
            ->assertSee('keepMarketingMediaDialogReachable', false)
            ->assertSee('class="text-info" id="marketingMediaWoNumber"', false)
            ->assertSee('id="marketingFileUploadForm"', false)
            ->assertSee('<option value="" selected>All files</option>', false)
            ->assertSee('#marketingFileCategory {', false)
            ->assertSee('border-color: rgba(var(--bs-warning-rgb), .58);', false)
            ->assertSee("marketingFileCategory?.addEventListener('change', applyManagerFileCategoryFilter)", false)
            ->assertSee("files.filter((file) => String(file.category || '') === category)", false)
            ->assertSee('const selectedCategory = marketingFileCategory.value;', false)
            ->assertSee('marketingFileCategory.value = selectedCategory;', false)
            ->assertSee('No files in this category', false)
            ->assertSee('Select a category for the files before uploading.', false)
            ...
            ->assertSee('id="marketingFileDropzone"', false)
            ->assertSee('Drag &amp; drop files here, or click to choose', false)
            ->assertSee("marketingFileDropzone?.addEventListener('drop'", false)
            ->assertSee('acceptMarketingDroppedFiles(event.dataTransfer.files)', false)
            ->assertSee('Choose no more than 10 files.', false)
            ->assertSee('the file is larger than 10 MB.', false)
            ->assertSee('Selected managers always receive an in-app notification. Check Send email notification to email the same managers too.', false)
