...
class ManualPartGroupController extends Controller
{
    public function store(Request $request, Manual $manual): JsonResponse
    {
        ...
        return response()->json([
            ...
            'group' => $this->serializeGroup($group->fresh(['options.coverages.component', 'serviceBulletin'])),
        ]);
    }
    ...
    public function update(Request $request, Manual $manual, ManualPartGroup $partGroup): JsonResponse
    {
        ...
        return response()->json([
            ...
            'group' => $this->serializeGroup($partGroup->fresh(['options.coverages.component', 'serviceBulletin'])),
        ]);
    }
    ...
    private function validatedData(Request $request, Manual $manual): array
    {
        ...
        if ($behavior === ManualPartGroup::BEHAVIOR_BUNDLE && trim((string) ($data['order_part_number'] ?? '')) === '') {
            throw ValidationException::withMessages(['order_part_number' => 'Order P/N is required for ASSY and SB KIT.']);
        }
        if ($data['type'] === ManualPartGroup::TYPE_SB_KIT && empty($data['manual_service_bulletin_id'])) {
            throw ValidationException::withMessages(['manual_service_bulletin_id' => 'Service Bulletin is required for SB KIT.']);
        }
    ...
    private function replaceOptions(ManualPartGroup $group, array $data, bool $preserveExisting): void
    {
        ...
        $keptCoverageIds = [];
        $existingCoverages = $option->coverages()->get()->keyBy('component_id');

        foreach ($data['component_ids'] as $componentId) {
            $coverage = $existingCoverages->get($componentId) ?: $option->coverages()->make([
                'component_id' => $componentId,
            ]);
            $coverage->fill([
