...
function updateNdtTable(components) {
    ...
    sortedComponents.forEach((component, displayIndex) => {
        ...
        const row = `<tr data-index="${originalIndex}" data-display-index="${displayIndex}"><td>${component.ipl_num}</td><td>${component.part_number}</td><td>${component.description}</td><td>${component.process}</td><td>${component.qty}</td><td><button class="btn btn-sm btn-primary me-1" onclick="editNdtComponent(${originalIndex})" title="Edit"><i class="fas fa-edit"></i></button><button class="btn btn-sm btn-danger" onclick="removeNdtComponent(${originalIndex})" title="Delete"><i class="fas fa-trash"></i></button></td></tr>`;
        tbody.append(row);
    });
...
function updateCadTable(components) {
    ...
    sortedComponents.forEach((component, displayIndex) => {
        const originalIndex = components.indexOf(component);
        const row = `<tr data-index="${originalIndex}" data-display-index="${displayIndex}"><td>${component.ipl_num}</td><td>${component.part_number}</td><td>${component.description}</td><td>${component.process}</td><td>${component.qty}</td><td><button class="btn btn-sm btn-primary me-1" onclick="editCadComponent(${originalIndex})" title="Edit"><i class="fas fa-edit"></i></button><button class="btn btn-sm btn-danger" onclick="removeCadComponent(${originalIndex})" title="Delete"><i class="fas fa-trash"></i></button></td></tr>`;
        tbody.append(row);
    });
...
function updateStressTable(components) {
    ...
    sortedComponents.forEach((component, displayIndex) => {
        const originalIndex = components.indexOf(component);
        const row = `<tr data-index="${originalIndex}" data-display-index="${displayIndex}"><td>${component.ipl_num}</td><td>${component.part_number}</td><td>${component.description}</td><td>${component.process}</td><td>${component.qty}</td><td><button class="btn btn-sm btn-primary me-1" onclick="editStressComponent(${originalIndex})" title="Edit"><i class="fas fa-edit"></i></button><button class="btn btn-sm btn-danger" onclick="removeStressComponent(${originalIndex})" title="Delete"><i class="fas fa-trash"></i></button></td></tr>`;
        tbody.append(row);
    });
...
function updatePaintTable(components) {
    ...
    sortedComponents.forEach((component, displayIndex) => {
        const originalIndex = components.indexOf(component);
        const row = `<tr data-index="${originalIndex}" data-display-index="${displayIndex}"><td>${component.ipl_num}</td><td>${component.part_number}</td><td>${component.description}</td><td>${component.process}</td><td>${component.qty}</td><td><button class="btn btn-sm btn-primary me-1" onclick="editPaintComponent(${originalIndex})" title="Edit"><i class="fas fa-edit"></i></button><button class="btn btn-sm btn-danger" onclick="removePaintComponent(${originalIndex})" title="Delete"><i class="fas fa-trash"></i></button></td></tr>`;
        tbody.append(row);
    });
...
function initializeWhenReady() {
    if (typeof $ !== 'undefined') {
        ...
        $('#ndtForm').on('submit', function(e) {
            ...
            const data = { component_id: sc.val(), ipl_num: sc.data('ipl-num'), part_number: sc.data('part-number'), description: sc.data('description'), process: $('#ndtProcess').val(), qty: parseInt($('#ndtQty').val()), _token: $('meta[name="csrf-token"]').attr('content') };
            $.post(`{{ route('ndt-cad-csv.add-ndt', ['workorder' => $workorder->id]) }}`, data).done(function(r) {
                if (r.success) { ndtComponents.push(data); updateNdtTable(ndtComponents); $('#ndtModal').modal('hide'); $('#ndtForm')[0].reset(); $('#ndtComponent').val('').trigger('change'); showNotification('NDT component added', 'success'); } else showNotification('Error: ' + r.message, 'error');
        ...
        $('#cadForm').on('submit', function(e) {
            ...
            const data = { component_id: sc.val(), ipl_num: sc.data('ipl-num'), part_number: sc.data('part-number'), description: sc.data('description'), process: $('#cadProcess').val(), qty: parseInt($('#cadQty').val()), _token: $('meta[name="csrf-token"]').attr('content') };
            $.post(`{{ route('ndt-cad-csv.add-cad', ['workorder' => $workorder->id]) }}`, data).done(function(r) {
                if (r.success) { cadComponents.push(data); updateCadTable(cadComponents); $('#cadModal').modal('hide'); $('#cadForm')[0].reset(); $('#cadComponent').val('').trigger('change'); $('#cadProcess').val('').trigger('change'); showNotification('CAD component added', 'success'); } else showNotification('Error: ' + r.message, 'error');
        ...
        $('#paintForm').on('submit', function(e) {
            ...
            const data = { component_id: sc.val(), ipl_num: sc.data('ipl-num'), part_number: sc.data('part-number'), description: sc.data('description'), process: $('#paintProcess').val(), qty: parseInt($('#paintQty').val()), _token: $('meta[name="csrf-token"]').attr('content') };
            $.post(`{{ route('ndt-cad-csv.add-paint', ['workorder' => $workorder->id]) }}`, data).done(function(r) {
                if (r.success) { paintComponents.push(data); updatePaintTable(paintComponents); $('#paintModal').modal('hide'); $('#paintForm')[0].reset(); $('#paintComponent').val('').trigger('change'); $('#paintProcess').val('').trigger('change'); showNotification('Paint component added', 'success'); } else showNotification('Error: ' + r.message, 'error');
            }).fail(function() { showNotification('Error adding component', 'error'); });
        });
        ...
        $('#ndtEditForm').on('submit', function(e) {
            ...
            const data = { index: editIndex, part_number: $('#ndtEditPartNumber').val(), description: $('#ndtEditDescription').val(), process: $('#ndtEditProcess').val(), qty: parseInt($('#ndtEditQty').val()), _token: $('meta[name="csrf-token"]').attr('content') };
            $.post(`{{ route('ndt-cad-csv.edit-ndt', ['workorder' => $workorder->id]) }}`, data).done(function(r) {
                if (r.success) { ndtComponents[editIndex] = { ...ndtComponents[editIndex], part_number: data.part_number, description: data.description, process: data.process, qty: data.qty }; updateNdtTable(ndtComponents); $('#ndtEditModal').modal('hide'); showNotification('NDT component updated', 'success'); } else showNotification('Error: ' + r.message, 'error');
        ...
        $('#cadEditForm').on('submit', function(e) {
            ...
            if (!editIndex) { showNotification('Edit index not found', 'error'); return; }
            const data = { index: editIndex, part_number: $('#cadEditPartNumber').val(), description: $('#cadEditDescription').val(), process: $('#cadEditProcess').val(), qty: parseInt($('#cadEditQty').val()), _token: $('meta[name="csrf-token"]').attr('content') };
            $.post(`{{ route('ndt-cad-csv.edit-cad', ['workorder' => $workorder->id]) }}`, data).done(function(r) {
                if (r.success) { cadComponents[editIndex] = { ...cadComponents[editIndex], part_number: data.part_number, description: data.description, process: data.process, qty: data.qty }; updateCadTable(cadComponents); $('#cadEditModal').modal('hide'); showNotification('CAD component updated', 'success'); } else showNotification('Error: ' + r.message, 'error');
            }).fail(function() { showNotification('Error saving', 'error'); });
        });
        $('#paintEditForm').on('submit', function(e) {
            ...
            if (!editIndex) { showNotification('Edit index not found', 'error'); return; }
            const data = { index: editIndex, part_number: $('#paintEditPartNumber').val(), description: $('#paintEditDescription').val(), process: $('#paintEditProcess').val(), qty: parseInt($('#paintEditQty').val()), _token: $('meta[name="csrf-token"]').attr('content') };
            $.post(`{{ route('ndt-cad-csv.edit-paint', ['workorder' => $workorder->id]) }}`, data).done(function(r) {
                if (r.success) { paintComponents[editIndex] = { ...paintComponents[editIndex], part_number: data.part_number, description: data.description, process: data.process, qty: data.qty }; updatePaintTable(paintComponents); $('#paintEditModal').modal('hide'); showNotification('Paint component updated', 'success'); } else showNotification('Error: ' + r.message, 'error');
            }).fail(function() { showNotification('Error saving', 'error'); });
        });
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        if (response.type === 'ndt') { ndtComponents = response.components || []; updateNdtTable(ndtComponents); }
                        else if (response.type === 'cad') { cadComponents = response.components || []; updateCadTable(cadComponents); }
                        else if (response.type === 'paint') { paintComponents = response.components || []; updatePaintTable(paintComponents); }
                        else if (response.type === 'stress') { stressComponents = response.components || []; updateStressTable(stressComponents); }
...
window.reloadFromManual = function(type) {
    if (confirm('Reload ' + type.toUpperCase() + ' components from Manual CSV? This will replace all existing data.')) {
        $.post(`{{ route('ndt-cad-csv.reload-from-manual', ['workorder' => $workorder->id]) }}`, { type, _token: $('meta[name="csrf-token"]').attr('content') }).done(function(r) {
            if (r.success) { showNotification('Loaded ' + r.count + ' components', 'success'); location.reload(); } else showNotification('Error: ' + r.message, 'error');
        });
    }
};
...
window.forceLoadFromManual = function(type) {
    if (confirm('Force loading ' + type.toUpperCase() + ' components from Manual CSV?')) {
        $.post(`{{ route('ndt-cad-csv.force-load-from-manual', ['workorder' => $workorder->id]) }}`, { type, _token: $('meta[name="csrf-token"]').attr('content') }).done(function(r) {
            if (r.success) { showNotification('Loaded ' + r.count + ' components', 'success'); location.reload(); } else showNotification('Error: ' + r.message, 'error');
        });
    }
};

window.importNdtFromCsv = function() { $('#csvType').val('ndt'); $('#csvImportModal').modal('show'); };
window.importCadFromCsv = function() { $('#csvType').val('cad'); $('#csvImportModal').modal('show'); };
window.importPaintFromCsv = function() { $('#csvType').val('paint'); $('#csvImportModal').modal('show'); };
window.importStressFromCsv = function() { $('#csvType').val('stress'); $('#csvImportModal').modal('show'); };

window.editNdtComponent = function(index) {
    const c = ndtComponents[index];
    if (!c) { showNotification('Component not found', 'error'); return; }
    $('#ndtCurrentIpl').text(c.ipl_num); $('#ndtCurrentPartNumber').text(c.part_number); $('#ndtCurrentDescription').text(c.description); $('#ndtCurrentProcess').text(c.process); $('#ndtCurrentQty').text(c.qty);
    $('#ndtEditIndex').val(index); $('#ndtEditPartNumber').val(c.part_number); $('#ndtEditDescription').val(c.description); $('#ndtEditProcess').val(c.process); $('#ndtEditQty').val(c.qty);
...
window.editCadComponent = function(index) {
    ...
    if (!c) { showNotification('Component not found', 'error'); return; }
    $('#cadCurrentIpl').text(c.ipl_num); $('#cadCurrentPartNumber').text(c.part_number); $('#cadCurrentDescription').text(c.description); $('#cadCurrentProcess').text(c.process); $('#cadCurrentQty').text(c.qty);
    $('#cadEditIndex').val(index); $('#cadEditPartNumber').val(c.part_number); $('#cadEditDescription').val(c.description); $('#cadEditQty').val(c.qty);
...
window.editStressComponent = function(index) {
    ...
    if (!c) { showNotification('Component not found', 'error'); return; }
    $('#stressCurrentIpl').text(c.ipl_num); $('#stressCurrentPartNumber').text(c.part_number); $('#stressCurrentDescription').text(c.description); $('#stressCurrentProcess').text(c.process); $('#stressCurrentQty').text(c.qty);
    $('#stressEditIndex').val(index); $('#stressEditPartNumber').val(c.part_number); $('#stressEditDescription').val(c.description); $('#stressEditQty').val(c.qty);
