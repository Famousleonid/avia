                ...
                <form id="printSettingsForm">
                    ...
                    <div class="mb-4">
                        <h5 class="mb-3" data-bs-toggle="tooltip" 
                            ...
                            title="Настройки количества строк в таблице Stress Relief. Строки сверх лимита скрываются при печати. Настройки применяются автоматически при загрузке страницы."
                            data-tooltip-ru="Настройки количества строк в таблице Stress Relief. Строки сверх лимита скрываются при печати. Настройки применяются автоматически при загрузке страницы."
                            data-tooltip-en="Stress Relief table row settings. Rows exceeding the limit are hidden when printing. Settings are applied automatically on page load.">
                            📊 Tables
                        </h5>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                <label for="stressTableRows" class="form-label" data-bs-toggle="tooltip" 
                                        data-bs-placement="top" 
                                        title="Максимальное количество строк в таблице Stress Relief на одной странице. По умолчанию: 21 строка. Используется для всех страниц формы."
                                        data-tooltip-ru="Максимальное количество строк в таблице Stress Relief на одной странице. По умолчанию: 21 строка. Используется для всех страниц формы."
                                        data-tooltip-en="Maximum number of rows in Stress Relief table per page. Default: 21 rows. Used for all pages of the form.">
                                        ...
                                        <div class="row">
                                            <div class="col-md-4 mb-3">
                                                <label for="containerMaxWidth" class="form-label" data-bs-toggle="tooltip" 
                                                        data-bs-placement="top" 
                                                        title="Максимальная ширина контейнера с таблицей в пикселях. Рекомендуемое значение: 920px для Stress Relief формы. Увеличьте, если таблица слишком узкая."
                                                        data-tooltip-ru="Максимальная ширина контейнера с таблицей в пикселях. Рекомендуемое значение: 920px для Stress Relief формы. Увеличьте, если таблица слишком узкая."
                                                        data-tooltip-en="Maximum width of the table container in pixels. Recommended value: 920px for Stress Relief form. Increase if the table is too narrow.">
                                                    Max Width (px)
                                                </label>
                                                <div class="input-group">
...
<script>
    ...
    const PRINT_SETTINGS_KEY = 'stressFormStd_print_settings';
    const TOOLTIP_LANG_KEY = 'stressFormStd_tooltip_lang';
    ...
    window.savePrintSettings = function() {
        try {
            ...
            const settings = {
                ...
                stressTableRows: getValue('stressTableRows', '21', '')
            };
    ...
    function loadSettingsToForm(settings) {
        const elements = {
            ...
            'stressTableRows': { suffix: '', default: '21' }
        };
    ...
    function updateTooltipsLanguage(container, lang) {
        if (!container) {
            console.warn('Контейнер для tooltips не найден');
            return;
        }
        ...
        tooltipElements.forEach(function(el) {
            try {
                ...
                console.error('Ошибка при обновлении tooltip:', e);
            }
        });
    }
        ...
        if (modal) {
            modal.addEventListener('show.bs.modal', function() {
                try {
                    ...
                    console.error('Ошибка при загрузке настроек в модальное окно:', e);
                }
            });
        }
    });
