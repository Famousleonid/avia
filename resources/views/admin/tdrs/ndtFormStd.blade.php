            ...
            <div class="all-rows-container page-rows-container" style="--std-table-columns: 1fr 2.4fr 3.2fr 2fr 0.9fr 1.25fr 1.25fr;">
                ...
                @foreach($ndtPageRows as $ndtEntry)
                    ...
                    @elseif(($ndtEntry['kind'] ?? '') === 'data')
                        ...
                        @endphp
                        <div class="data-row-ndt std-grid-row {{ ! empty($component->group_crossed_out) ? 'std-group-crossed-out' : '' }}" data-row-index="{{ $ndtGlobalRowIndex }}" style="--std-row-min-height: {{ $rowHeight }}px;" @if(!empty($component->group_crossout_reason)) title="{{ $component->group_crossout_reason }}" @endif>
                            <div class="std-cell">
