...
<div id="dim-tab-wrap" class="m-2">
    ...
    <div id="dim-parts-panel">
        ...
        {{-- Fits & Clearances authoring (explicit OD↔ID pairs) --}}
        <div class="px-2 py-1 d-flex align-items-center gap-2" style="border-top:1px solid var(--bs-border-color)">
            <span class="fw-semibold" style="font-size:11px;text-transform:uppercase;letter-spacing:.05em;color:var(--bs-secondary-color)">Fits &amp; Clearances</span>
            <button type="button" class="btn btn-link btn-sm ms-auto p-0" style="font-size:11px" id="fcAddBtn" title="Add fit">
                <i class="bi bi-plus-lg"></i>
            </button>
        </div>
        <div id="fc-list" class="px-2 py-1" style="font-size:11px;max-height:240px;overflow:auto"></div>
        <div id="fc-form" class="px-2 py-2 border-top d-none" style="font-size:11px">
            <input type="hidden" id="fcEditId">
            <label class="d-block mb-1 text-secondary">OD member</label>
            <select id="fcOdSelect" class="form-select form-select-sm mb-2" style="font-size:11px"></select>
            <label class="d-block mb-1 text-secondary">ID member</label>
            <select id="fcIdSelect" class="form-select form-select-sm mb-2" style="font-size:11px"></select>
            <label class="d-block mb-1 text-secondary">Ref. No <span class="text-secondary">(optional)</span></label>
            <input id="fcRefNo" class="form-control form-control-sm mb-2" style="font-size:11px" placeholder="e.g. 8001-1">
            <div class="row g-1 mb-2">
                <div class="col-6"><label class="d-block text-secondary" style="font-size:10px">Asm clr min</label><input id="fcAsmMin" class="form-control form-control-sm" style="font-size:11px" placeholder="auto"></div>
                <div class="col-6"><label class="d-block text-secondary" style="font-size:10px">Asm clr max</label><input id="fcAsmMax" class="form-control form-control-sm" style="font-size:11px" placeholder="auto"></div>
            </div>
            <label class="d-block mb-1 text-secondary">Permitted clr</label>
            <input id="fcPerm" class="form-control form-control-sm mb-2" style="font-size:11px" placeholder="auto">
            ...
            <div class="text-secondary mt-1" style="font-size:10px">Leave clearances blank to derive from member limits.</div>
        </div>
    </div>
...
{{-- Fits & Clearances panel — self-contained CRUD (does not touch the dimensions script scope) --}}
<script>
(function () {
    const MANUAL_ID = {{ $cmm->id }};
    ...
    function renderList() {
        if (!fits.length) { listEl.innerHTML = '<div class="text-secondary">No fits yet.</div>'; return; }
        listEl.innerHTML = fits.map(f => {
            ...
            return '<div class="d-flex align-items-start gap-1 py-1 border-bottom">'
                ...
                +   '<div class="fw-semibold">' + esc(f.ref_no || '—') + warn + '</div>'
                +   '<div class="text-truncate">OD: ' + esc(f.od_label || ('#' + f.od_param_id)) + '</div>'
                +   '<div class="text-truncate">ID: ' + esc(f.id_label || ('#' + f.id_param_id)) + '</div>'
                +   '<div class="text-secondary">asm ' + fmt(f.eff_assembly_min) + '–' + fmt(f.eff_assembly_max) + ' · perm ' + fmt(f.eff_permitted) + '</div>'
                + '</div>'
    ...
    async function save() {
        ...
        try {
            if (editId.value) {
                await api('/fits/' + editId.value, { method: 'PATCH', body: JSON.stringify(payload) });
            } else {
                await api('/manuals/' + MANUAL_ID + '/fits', { method: 'POST', body: JSON.stringify(payload) });
            }
            hideForm();
            await loadAll();
        } catch (e) {
    ...
    function init() { loadAll().catch(e => { listEl.innerHTML = '<div class="text-danger">' + esc(e.message) + '</div>'; }); }
