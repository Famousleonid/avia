...
function buildQuantumRoQuery(array $params): array
{
    ...
    // - PN = NDTB, Machining, CADB, CAD Plate B, Anodizing, Passivation means bushing batch bucket
...
SELECT
    ...
    CASE
        ...
        WHEN REPLACE(REPLACE(REPLACE(UPPER(TRIM(pm_rd.PN)), ' ', ''), '-', ''), '_', '') IN ('CAD', 'CADPLATE')
            THEN 'STD_LIST_CAD'
        WHEN REPLACE(REPLACE(REPLACE(UPPER(TRIM(pm_rd.PN)), ' ', ''), '-', ''), '_', '') IN ('NDTB', 'MACHINING', 'CADB', 'CADPLATEB', 'ANODIZING', 'ANODISING', 'PASSIVATION')
