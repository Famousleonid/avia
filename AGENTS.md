...
- PRL XLS/XLSX: set a new KIT checkbox only when the matched row's CODE cell explicitly contains `KIT` (trimmed, case-insensitive). Blank CODE, including trailing PRL rows, means no new KIT mark. Never invent or carry down KIT codes, including during IPL/P/N remapping or SQL generation; ASSY/KIT group membership is not evidence for this checkbox. Report unsupported existing KIT flags separately; do not clear them or rewrite prior imports without a scoped reconciliation request.
- Always include the IPL quantity in imported Parts (`units_assy`) and generated SQL: retain numeric values; use `1` for letters (including AR/RF) or blank values. Preserve raw source tokens for audit and verify normalized quantities after import. Do not retroactively change existing manuals without a scoped import/reconciliation request.
- The standard source root is `C:\Airplane`, with one folder per manual containing numbered FIG PDFs and the process XLS/XLSX workbook. Confirm the manual identity from the documents and production data. Save each versioned final import SQL and SELECT-only verification SQL in that manual's source folder; keep intermediate artifacts in the workspace run directory. The user executes production SQL; verify the result read-only afterwards.
...
- Treat `.agents/skills/avia-import-manual-package/references/processed-manuals.md` as the canonical persistent ledger of manual-package work. Read it before answering which manuals or figures have already been processed.
- After every PDF Parts extraction, Part Group/ASSY build, XLS/XLSX process mapping, production handoff, or production verification, update that ledger in the same task. Record the exact manual number and ID, source/run artifacts, per-FIG Parts and group coverage, workbook flags/SB counts, verification date, unresolved discrepancies, and the current status.
- Mark a manual `complete` only when all supplied PDF figures have been reconciled into Parts, all required ASSY/Alternative P/N/Original-Oversize/KIT groups for those figures have been audited, workbook flags and Service Bulletins have zero unresolved conflicts, and the post-import production SELECT audit passes. Otherwise use `partial` and state the missing stage or figures; the presence of Parts alone never proves completion.
- Present manual progress as a table covering every requested manual, with per-FIG columns or expandable per-FIG details and a final completion column. Use `✓` only for evidenced completion, `?` for unknown completion, and explicitly distinguish empty production Parts from work that is known never to have started. When the user requests production-only statistics, do not reuse local ledger completion labels as production facts; show what production can establish.
