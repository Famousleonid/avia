...
$scratch='avia_m58_review_20260918_'.bin2hex(random_bytes(4));
...
$target=current(array_filter($snap['manuals'],fn($m)=>$m['id']===58));
...
function fixture(PDO $pdo,array $snap,array $target,bool $checkSheet=false):void{
 if(!preg_match('/^avia_m58_review_20260918_[a-f0-9]{8}$/',(string)$pdo->query('SELECT DATABASE()')->fetchColumn())){throw new RuntimeException('Not scratch');}
 ...
 seed($pdo,'manuals',[['id'=>58,'number'=>'32-10-02','deleted_at'=>null],['id'=>59,'number'=>'UNRELATED','deleted_at'=>null]]);
...
$sql=file_get_contents(__DIR__.'/32-10-02_20260918_v01_import.sql');$verify=file_get_contents(__DIR__.'/32-10-02_20260918_v01_verify.sql');$report=[];
try{
 ...
 $dbGroups=App\Models\ManualPartGroup::where('manual_id',58)->with('options.coverages')->get()->keyBy('id');$opts=$dbGroups->flatMap(fn($g)=>$g->options)->keyBy('id');
 $ids=$pdo->query('SELECT ipl_num,id FROM components WHERE manual_id=58 AND deleted_at IS NULL')->fetchAll(PDO::FETCH_KEY_PAIR);
 ...
 $pdo->exec('UPDATE components SET log_card=1,paint_list=1,kit=1 WHERE id=605');
 $retained=runSql($pdo,$sql);
 if(result($retained)!=='SUCCESS'||(int)$pdo->query('SELECT log_card+paint_list+kit FROM components WHERE id=605')->fetchColumn()!==3){throw new RuntimeException('Existing true flags lost');}
 ...
 foreach(['wrong_manual','changed_pn','missing_id','foreign_code','template_conflict','postflight_corruption','continue_after_dml_error'] as $case){
  ...
  if($case==='wrong_manual'){$pdo->exec("UPDATE manuals SET number='WRONG' WHERE id=58");}
  if($case==='changed_pn'){$pdo->exec("UPDATE components SET part_number='WRONG' WHERE id=605");}
  if($case==='missing_id'){$pdo->exec('DELETE FROM components WHERE id=605');}
  if($case==='foreign_code'){$pdo->exec("INSERT INTO manual_part_groups(manual_id,code,name,type,behavior) VALUES(59,'MPG-M58-F1-ASSY-1','foreign','assy','bundle')");}
  if($case==='template_conflict'){$pdo->exec("INSERT INTO manual_in_process_check_sheets(manual_id,source_file,source_sheet,source_sha256,content_sha256,content) VALUES(58,'different','different',REPEAT('a',64),REPEAT('b',64),'{}')");}
  if($case==='postflight_corruption'){$candidate=str_replace('-- POSTFLIGHT START',"UPDATE components SET units_assy='999' WHERE id=605;\n-- POSTFLIGHT START",$candidate);}
