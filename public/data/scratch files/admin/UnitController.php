...
//    }
    ...
    // -----------------------  New release  ------------------------
    public function getUnits($manualId)
    {
        $units = Unit::where('manual_id', $manualId)->get();
    ...
    public function updateUnits(Request $request, $manualId)
    {
        $validated = $request->validate([
            'part_numbers.*.part_number' => 'required|string|max:255',
            'part_numbers.*.verified' => 'required|boolean',
        ]);

        $partNumbers = $validated['part_numbers'];

        // Удаляем старые записи
        Unit::where('manual_id', $manualId)->delete();
